...
def _masked_audio_to_tempfile(y_masked: np.ndarray, sr: int) -> str:
    ...
    # subtype FLOAT evita quantizzazione PCM inutile
...
def analyze_dime(
    ...
    audio_factorization_cache = None
    audio_factorization_meta_cache = None
    ...
    for k_tok, cap_tok in enumerate(caption_tokens):
        ...
        # -----------------------
        if _DIME_AUDIO_FEATURE_MODE == "audiolime_demucs" and audio_factorization_cache is not None:
            factorization = audio_factorization_cache
            meta_base = dict(audio_factorization_meta_cache)
            n_audio = int(factorization.get_number_components())
            duration = float(len(y0_full) / sr0)

            A, am_list, sidx_a = make_audiolime_binary_masks(
                n_components=n_audio,
            ...
            X_a = A
            audio_lime_meta = dict(meta_base)
            audio_lime_meta["factorization_obj"] = factorization
            ...
            if _DIME_AUDIO_FEATURE_MODE == "audiolime_demucs" and audio_factorization is not None:
                audio_factorization_cache = audio_factorization
                audio_factorization_meta_cache = {
                    k: v for k, v in audio_lime_meta.items() if k != "factorization_obj"
                }
            ...
            try:
                for s in batch_indices:
                    ...
                    if save_debug_audio and debug_audio_dir:
                        ...
                        if s < 5:  # salva solo pochi esempi
                            dbg_path = os.path.join(
                                debug_audio_dir,
                                f"tok{k_tok:03d}_sample{s:03d}.wav"
                            )
                            try:
                ...
                if runner is not None:
                    ...
                    with torch.inference_mode():
                        for ap in pert_audio_paths:
                            row = [
                                float(dime_token_value(model, processor, ap, p, caption_ids, k_tok))
                                for p in bg_prompts
                            ]
    ...
    # Caption side: token -> word aggregation
    # Prompt side: già word-level nel nuovo masking DIME
    # ==========================================================
    ...
    if do_wordlevel:
        ...
        # Prompt side: se feature_unit=word, NON aggregare di nuovo
        uc2_global_word = (
        ...
        wordlevel = {
            ...
            "prompt": {
                ...
                "words": [str(x) for x in (prompt_feature_labels or [])],  # compat
