"""
Experiment 1b — Silent Audio Baseline for Attention Pattern (HumMusQA appendix)
===============================================================================
Identical to Exp1 (exp1_attention_pattern_hummusqa.py) in every respect, EXCEPT
that a pre-existing silent WAV file is passed to the model for every evaluation
instead of the real audio from the HumMusQA dataset.

Purpose (appendix control condition)
-------------------------------------
Exp1 found that audio tokens receive ≤ 3% of answer-token attention in late layers
(text/audio ratio 42.8× at layer 27). This experiment asks: is that pattern
structural (appears regardless of audio content) or content-driven?

  • If silence → same attention profile + accuracy ≈ 66%  →  the model responds
    from text alone; audio content is irrelevant to its decision-making process.
  • If silence → accuracy drops to ~25% (chance)  →  the audio does carry
    information even if it receives little explicit attention.

The single change vs. Exp1 is the use of `audio_path_override` in prepare_inputs():
every sample passes `silence_path` instead of materialising the HF audio bytes.
All hooks, aggregation, checkpointing, and output format are unchanged, so
merge_exp1_shards.py and plot_exp1_attention_pattern.py work without modification.

DO NOT MODIFY exp1_attention_pattern_hummusqa.py — this is an independent file.

Silence WAV
-----------
Provide the path to a pre-existing silent WAV via --silence_path (required).
The file is reused for every evaluation; it is never deleted.

Usage
-----
# Quick test (4 samples, 1 permutation)
python -m QA_analysis.experiments.Exp_Net.exp1b_silence_baseline_hummusqa \\
    ...
    --output_dir  /tmp/exp1b_silence_test \\
    --device_map  cuda:0 \\
    --silence_path /nas/home/fingenito/Thesis_project/QA_analysis/data/audio_silence.wav \\
    --max_samples 4 --option_permutations 1 --fail_fast

# Full run (via launch_exp1b_parallel.sh)
bash QA_analysis/experiments/Exp_Net/launch_exp1b_parallel.sh \\
    8 320 \\
    /nas/home/fingenito/Thesis_project/QA_analysis/Results_QA/experiments/exp_Net/exp1_attention/silence_baseline_7b_3perm_parallel \\
    3 /nas/home/fingenito/Models/Qwen2.5-Omni-7B 0
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Path to a pre-existing silent WAV file. This file is passed to the model "
            "for EVERY evaluation instead of the real HumMusQA audio. "
            "The file is never deleted by this script. "
            "Example: /nas/home/fingenito/Thesis_project/QA_analysis/data/audio_silence.wav"
        ),
    )
...
def main() -> None:
    ...
    # ------------------------------------------------------------------
    # Validate silence path before doing anything else
    # ------------------------------------------------------------------
    if not os.path.exists(args.silence_path):
        raise FileNotFoundError(
            f"Silence WAV not found: {args.silence_path}\n"
            "Provide a valid path with --silence_path."
        )

    print("\n" + "=" * 78)
    print("Experiment 1b — Silent Audio Baseline | HumMusQA audio-text MCQ (appendix)")
    print(f"Model      : {args.model_path}")
    print(f"Output dir : {args.output_dir}")
    print(f"Mode       : {args.attention_mode}")
    print(f"🔇 Silence : {args.silence_path}  (used for ALL evaluations)")
    ...
    for evaluation_idx, (sample_idx, sample, option_permutation_idx, option_order) in enumerate(
        ...
        try:
            ...
            inputs, correct_letter, audio_path, should_delete, rendered_text, text_spans = prepare_inputs(
                sample, sample_idx, processor, tmp_dir,
                audio_path_override=args.silence_path,  # exp1b: always use silent audio
    ...
    save_results(
        ...
        args.output_dir,
        {
            ...
            "errors": errors,
            "methodological_note": (
                "Experiment 1b — silent audio control condition for HumMusQA attention pattern analysis. "
                "Identical to Experiment 1 in all respects EXCEPT that a single pre-existing silent WAV "
                "file is substituted for the real HumMusQA audio on every evaluation. "
                "The silence file is passed via prepare_inputs(audio_path_override=silence_path), "
                "which bypasses materialize_audio() and sets should_delete=False so the shared "
                "silence file is never removed between evaluations. "
                "Model: Qwen2_5OmniForConditionalGeneration with "
                ...
                "towards the multimodal prompt (with silent audio) is extracted from the hook storage. "
                ...
                "Output schema is identical to Experiment 1 — merge_exp1_shards.py and "
                "plot_exp1_attention_pattern.py work without modification."
            ),
            "important_note": (
                "Experiment 1b measures answer-token attention when the audio input is silent (all zeros). "
                "Comparing to Experiment 1 (real audio) reveals whether the attention distribution "
                "is content-driven or a structural property of the architecture. "
                "If accuracy with silence ≈ 66% (same as real audio), the model responds from text alone."
            ),
        },
        total_layers,
    )
