...
EXPERIMENT_RESULTS_ROOT = "/nas/home/fingenito/Thesis_project/QA_analysis/Results_paper/experiments"
MODEL_PATH = "/nas/home/fingenito/Models/Qwen2.5-Omni-7B"

# audio_only: la domanda reale di ogni sample viene sostituita da questo
# unico placeholder, cosi' l'unica informazione che arriva al modello e'
...
def run_batch_exp_a(
    ...
    for orig_idx, entry in valid_entries:
        ...
        # audio_only: solo la domanda viene mascherata, le opzioni restano vere.
        meta["question"] = NULL_QUESTION
        try:
    ...
    try:
        for batch_pos, (orig_idx, entry) in enumerate(valid_entries):
            ...
            try:
                ...
                # audio_only: l'audio resta reale e vario, quindi il pool di
                # background audio si costruisce come nel caso base.
                ...
                # La domanda e' mascherata per tutti i sample: pescare i
                # prompt di background da all_prompts rischierebbe di
                # ripescare stringhe identiche (stesso [MASK] + eventuali
                # opzioni duplicate tra sample) e far collassare il pool per
                # dedup, come successo per l'audio in text_only. Costruiamo
                # quindi un pool dedicato di placeholder sintatticamente
                # distinti ma ugualmente privi di informazione, con le
                # opzioni vere di QUESTO sample.
                bg_prompts = [
                    build_hummusqa_qwen25_prompt(f"{NULL_QUESTION}_{i}", meta["options"])
